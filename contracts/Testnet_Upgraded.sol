...
contract Upgraded {
    ...
    function getDownlines(
        ...
        try Pro.downlines(packageIndex, user, pos) returns(address result){
            return result;
        }catch{
    ...
    function getSecDownlines(
        ...
        try Pro.secondLayerDownlines(packageIndex, user, pos) returns(address result){
            return result;
        }catch{
            return 0x0000000000000000000000000000000000000001;
        }  
...
function getAllDownlines(
    ...
    for (uint256 pos = 0; pos < 4; pos++) {
        address result = getDownlines(packageIndex, user, pos);
        if (result == 0x0000000000000000000000000000000000000001) {
            break; // Exit the loop if getDownlines returns 0x0000000000000000000000000000000000000001
        }
        allResults = appendToAddressArray(allResults, result);
    }

    return allResults; // Return the array of results
}

function appendToAddressArray(address[] memory array, address element) private pure returns (address[] memory) {
...
function getAllSecDownlines(
    ...
    for (uint256 pos = 0; pos < 4; pos++) {
        address result = getSecDownlines(packageIndex, user, pos);
        if (result == 0x0000000000000000000000000000000000000001) {
            break; // Exit the loop if getDownlines returns 0x0000000000000000000000000000000000000001
        }
        allResults = appendToSecAddressArray(allResults, result);
    }

    return allResults; // Return the array of results
}

function appendToSecAddressArray(address[] memory array, address element) private pure returns (address[] memory) {
    address[] memory newArray = new address[](array.length + 1);
    for (uint256 i = 0; i < array.length; i++) {
        newArray[i] = array[i];
    }
    ...
    for (uint256 i = 0; i < users.length; i++) {
        address user = users[i];
        uint pack = newUserPackages[users[i]];

        for (uint256 i = 1; i <= pack; i++) {

        address[] memory downline = getAllDownlines(packageIndex, user);
        newDownlines[packageIndex][user] = downline;
if(downline.length != 0){
        address[] memory secdownline = getAllSecDownlines(packageIndex, user);
        newDownlines[packageIndex][user] = secdownline;
}
    }
    }
}
    ...
    function purchasePackage(uint256 packageIndex) external{
        ...
        if (newSecondLayerDownlines[packageIndex][structureUpline2].length == 16) {
            recycleProvidePackage(packageIndex, remainingAmount, structureUpline2);
        }
